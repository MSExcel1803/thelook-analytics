...
**Lineage.** Sources through staging and intermediate models into the marts and
the declared exposures. Generated by `dbt docs generate` — not drawn by hand, so
it cannot drift from the code.

![dbt lineage graph for the full project](docs/dbt-dag.png)

**CI refusing a bad merge.** A pull request that dropped a valid value from an
`accepted_values` test. The test failed, the check is marked **Required**, and the
merge button is disabled. `main` cannot accept data that fails its tests — this is
the difference between running tests and enforcing them.

![A pull request blocked from merging because a dbt data test failed](docs/ci-blocks-bad-data.png)
...
bigquery-public-data.thelook_ecommerce   (sources + freshness checks)
    ...
    └──────────────────────────────────────────────────────┘
                ...
                ▼
    ┌──────────────────────────┐
    │  semantic/  (MetricFlow) │  23 metrics defined once.
    └──────────────────────────┘
                │
                ▼
